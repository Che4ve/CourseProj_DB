---
name: Миграция в монорепозиторий (ФИНАЛЬНАЯ)
overview: "Переделка Next.js + Supabase проекта в монорепозиторий с NestJS backend, Prisma ORM, PostgreSQL. ИСПРАВЛЕНО: правильный подсчёт \"5 РАЗНЫХ типов\" (UUID≠TEXT≠INTEGER≠BOOLEAN≠TIMESTAMPTZ), параметризация в Prisma middleware без конкатенации."
todos:
  - id: phase0-docs
    content: Создать docs/requirements-checklist.md
    status: completed
  - id: phase1-monorepo-setup
    content: Настроить pnpm workspaces + Turborepo
    status: completed
  - id: phase1-move-frontend
    content: Переместить Next.js в apps/frontend
    status: completed
    dependencies:
      - phase1-monorepo-setup
  - id: phase1-cleanup
    content: Удалить тесты, vitest, package-lock.json
    status: completed
    dependencies:
      - phase1-move-frontend
  - id: phase2-db-schema-types
    content: Создать schema.prisma с 13 таблицами, КАЖДАЯ >= 5 РАЗНЫХ типов
    status: completed
  - id: phase2-manual-migrations
    content: Добавить _manual_migrations в schema
    status: completed
    dependencies:
      - phase2-db-schema-types
  - id: phase2-sql-triggers
    content: Создать 001_triggers.sql (аудит с user_id + агрегаты)
    status: completed
    dependencies:
      - phase2-db-schema-types
  - id: phase2-sql-functions
    content: Создать 002_functions.sql (скалярная + табличная)
    status: completed
    dependencies:
      - phase2-db-schema-types
  - id: phase2-sql-views
    content: Создать 003_views.sql (3 VIEW)
    status: completed
    dependencies:
      - phase2-db-schema-types
  - id: phase2-sql-indexes
    content: Создать 004_indexes.sql
    status: completed
    dependencies:
      - phase2-db-schema-types
  - id: phase2-apply-sql-script
    content: Реализовать apply-sql.ts с контролем
    status: completed
    dependencies:
      - phase2-manual-migrations
  - id: phase2-seed
    content: "Seed: 800 habits, 10000 checkins, faker.seed(12345)"
    status: completed
    dependencies:
      - phase2-db-schema-types
  - id: phase3-backend-init
    content: Создать NestJS в apps/backend
    status: completed
  - id: phase3-prisma-middleware-safe
    content: "ПРИОРИТЕТ: Prisma middleware с ПАРАМЕТРИЗАЦИЕЙ (set_config или $executeRaw)"
    status: completed
    dependencies:
      - phase3-backend-init
      - phase2-db-schema-types
  - id: phase3-auth
    content: Auth + JWT + bcrypt + AsyncLocalStorage
    status: completed
    dependencies:
      - phase3-backend-init
  - id: phase3-crud
    content: CRUD эндпойнты через Prisma
    status: completed
    dependencies:
      - phase3-auth
      - phase2-db-schema-types
  - id: phase3-reports-safe-sql
    content: Отчёты с $queryRaw (ПАРАМЕТРИЗОВАННЫЙ SQL)
    status: completed
    dependencies:
      - phase3-crud
  - id: phase3-batch
    content: Batch import с логированием
    status: completed
    dependencies:
      - phase3-crud
  - id: phase3-swagger
    content: Swagger /api/docs
    status: completed
    dependencies:
      - phase3-backend-init
  - id: phase4-remove-supabase
    content: Удалить Supabase зависимости
    status: completed
    dependencies:
      - phase1-move-frontend
  - id: phase4-api-client
    content: Создать API клиент
    status: completed
    dependencies:
      - phase4-remove-supabase
  - id: phase4-rewrite-actions
    content: Переписать server actions
    status: completed
    dependencies:
      - phase4-api-client
      - phase3-crud
  - id: phase4-middleware-no-fetch
    content: Middleware БЕЗ сетевых вызовов (jwtVerify)
    status: completed
    dependencies:
      - phase4-api-client
  - id: phase5-docker
    content: docker-compose.yml
    status: completed
  - id: phase5-env-docker-fix
    content: ".env: backend:3001 для Docker, localhost:3001 для local"
    status: completed
    dependencies:
      - phase5-docker
  - id: phase6-perf-analysis
    content: EXPLAIN ANALYZE + docs/perf.md
    status: completed
    dependencies:
      - phase2-sql-indexes
  - id: phase7-schema-doc-types
    content: docs/schema.md с подсчётом типов для каждой таблицы
    status: completed
    dependencies:
      - phase2-db-schema-types
  - id: phase7-docs
    content: Остальные docs/
    status: completed
    dependencies:
      - phase0-docs
  - id: phase7-readme
    content: README с quick start
    status: completed
    dependencies:
      - phase7-docs
  - id: final-check-types
    content: "ПРОВЕРКА: >= 5 разных типов в каждой таблице"
    status: completed
    dependencies:
      - phase7-schema-doc-types
  - id: final-check-no-concat
    content: "ПРОВЕРКА: grep нет SQL-конкатенации в backend"
    status: completed
    dependencies:
      - phase3-prisma-middleware-safe
      - phase3-reports-safe-sql
---

# План миграции проекта в монорепозиторий (ФИНАЛЬНАЯ ВЕРСИЯ)

## Фаза 0: Подготовка и документация требований

**Создание чеклиста требований курсовой**Создать [`docs/requirements-checklist.md`](docs/requirements-checklist.md) с полным чеклистом из PDF и картой соответствия "требование → реализация".

## Фаза 1: Структура монорепозитория

**Настройка pnpm workspaces + Turborepo**Создать в корне:

- [`pnpm-workspace.yaml`](pnpm-workspace.yaml) — определение воркспейсов
- [`turbo.json`](turbo.json) — конфигурация Turborepo
- Новый корневой [`package.json`](package.json) с workspaces-скриптами
- [`.npmrc`](.npmrc)

**Миграция фронтенда в apps/frontend**Переместить все Next.js файлы в [`apps/frontend/`](apps/frontend/):

- `app/`, `components/`, `contexts/`, `lib/`, `public/`
- `middleware.ts`, `next.config.ts`, `tsconfig.json`, `postcss.config.mjs`
- `biome.json`, `components.json`, `tailwind.config.ts`

**Очистка**Удалить: `__tests__/`, `vitest.config.ts`, `vitest.setup.ts`, `package-lock.json`, зависимости vitest/testing-library

## Фаза 2: Проектирование и реализация БД (packages/db)

**Структура packages/db**

```javascript
packages/db/
├── prisma/
│   ├── schema.prisma
│   └── migrations/
├── sql/
│   ├── 001_triggers.sql
│   ├── 002_functions.sql
│   ├── 003_views.sql
│   └── 004_indexes.sql
├── scripts/
│   ├── seed.ts
│   └── apply-sql.ts
├── package.json
└── tsconfig.json
```

**СХЕМА БД (13 ТАБЛИЦ) — ИСПРАВЛЕНО: ПРАВИЛЬНЫЙ ПОДСЧЁТ ТИПОВ**

### Правило подсчёта:

- TEXT x3 = **1 тип** (TEXT)
- UUID x2 = **1 тип** (UUID)
- TIMESTAMPTZ x2 = **1 тип** (TIMESTAMPTZ)

Нужно: минимум 5 **уникальных** типов в каждой таблице.---

### 1. users — 8 колонок, 6 РАЗНЫХ типов ✓

```sql
id                UUID PRIMARY KEY DEFAULT gen_random_uuid()
email             TEXT UNIQUE NOT NULL
password_hash     TEXT NOT NULL
full_name         TEXT NOT NULL
role              VARCHAR(50) DEFAULT 'user'
is_active         BOOLEAN DEFAULT true
created_at        TIMESTAMPTZ DEFAULT NOW()
login_count       INTEGER DEFAULT 0
```

**Уникальные типы:** UUID, TEXT, VARCHAR, BOOLEAN, TIMESTAMPTZ, INTEGER = **6 типов** ✓---

### 2. user_profiles (1:1 с users) — 8 колонок, 6 РАЗНЫХ типов ✓

```sql
id                    UUID PRIMARY KEY DEFAULT gen_random_uuid()
user_id               UUID UNIQUE NOT NULL REFERENCES users ON DELETE CASCADE
bio                   TEXT
avatar_url            TEXT
timezone              VARCHAR(100) DEFAULT 'UTC'
date_of_birth         DATE
notification_enabled  BOOLEAN DEFAULT true
theme_preference      SMALLINT DEFAULT 0 CHECK(theme_preference >= 0 AND theme_preference <= 2)
```

**Уникальные типы:** UUID, TEXT, VARCHAR, DATE, BOOLEAN, SMALLINT = **6 типов** ✓---

### 3. habits (1:N от users) — 10 колонок, 7 РАЗНЫХ типов ✓

```sql
id              UUID PRIMARY KEY DEFAULT gen_random_uuid()
user_id         UUID NOT NULL REFERENCES users ON DELETE CASCADE
name            TEXT NOT NULL
description     TEXT
type            VARCHAR(10) CHECK(type IN ('good', 'bad')) NOT NULL
color           VARCHAR(7) DEFAULT '#6366f1'
priority        SMALLINT DEFAULT 0 CHECK(priority >= 0 AND priority <= 10)
is_archived     BOOLEAN DEFAULT false
display_order   INTEGER DEFAULT 0
created_at      TIMESTAMPTZ DEFAULT NOW()
```

**Уникальные типы:** UUID, TEXT, VARCHAR, SMALLINT, BOOLEAN, INTEGER, TIMESTAMPTZ = **7 типов** ✓---

### 4. habit_schedules (1:N от habits) — 8 колонок, 7 РАЗНЫХ типов ✓

```sql
id                UUID PRIMARY KEY DEFAULT gen_random_uuid()
habit_id          UUID NOT NULL REFERENCES habits ON DELETE CASCADE
frequency_type    VARCHAR(20) CHECK(frequency_type IN ('daily', 'weekly', 'monthly', 'custom'))
frequency_value   INTEGER DEFAULT 1 CHECK(frequency_value > 0)
weekdays_mask     SMALLINT DEFAULT 127 -- битовая маска для дней недели
start_date        DATE NOT NULL
end_date          DATE
is_active         BOOLEAN DEFAULT true
```

**Уникальные типы:** UUID, VARCHAR, INTEGER, SMALLINT, DATE, BOOLEAN = **6 типов** ✓---

### 5. habit_checkins (ТРАНЗАКЦИОННАЯ, 10000+) — 9 колонок, 7 РАЗНЫХ типов ✓

```sql
id                  UUID PRIMARY KEY DEFAULT gen_random_uuid()
habit_id            UUID NOT NULL REFERENCES habits ON DELETE CASCADE
user_id             UUID NOT NULL REFERENCES users ON DELETE CASCADE
checkin_date        DATE NOT NULL
checkin_time        TIME DEFAULT CURRENT_TIME
notes               TEXT
mood_rating         SMALLINT CHECK(mood_rating >= 1 AND mood_rating <= 5)
duration_minutes    INTEGER
created_at          TIMESTAMPTZ DEFAULT NOW()

UNIQUE(habit_id, checkin_date)
```

**Уникальные типы:** UUID, DATE, TIME, TEXT, SMALLINT, INTEGER, TIMESTAMPTZ = **7 типов** ✓---

### 6. tags — 7 колонок, 6 РАЗНЫХ типов ✓

```sql
id              UUID PRIMARY KEY DEFAULT gen_random_uuid()
name            VARCHAR(100) UNIQUE NOT NULL
slug            TEXT UNIQUE NOT NULL  -- для URL-friendly имени
color           VARCHAR(7) DEFAULT '#gray'
usage_count     INTEGER DEFAULT 0
is_system       BOOLEAN DEFAULT false  -- системные теги нельзя удалить
created_at      TIMESTAMPTZ DEFAULT NOW()
```

**Уникальные типы:** UUID, VARCHAR, TEXT, INTEGER, BOOLEAN, TIMESTAMPTZ = **6 типов** ✓---

### 7. habit_tags (N:M habits↔tags) — 7 колонок, 6 РАЗНЫХ типов ✓

```sql
id              UUID PRIMARY KEY DEFAULT gen_random_uuid()
habit_id        UUID NOT NULL REFERENCES habits ON DELETE CASCADE
tag_id          UUID NOT NULL REFERENCES tags ON DELETE CASCADE
priority        SMALLINT DEFAULT 0
is_primary      BOOLEAN DEFAULT false
assigned_by     VARCHAR(50) DEFAULT 'user'  -- 'user' или 'auto'
assigned_at     TIMESTAMPTZ DEFAULT NOW()

UNIQUE(habit_id, tag_id)
```

**Уникальные типы:** UUID, SMALLINT, BOOLEAN, VARCHAR, TIMESTAMPTZ = **5 типов** ✓---

### 8. reminders (1:N от habits) — 8 колонок, 7 РАЗНЫХ типов ✓

```sql
id                  UUID PRIMARY KEY DEFAULT gen_random_uuid()
habit_id            UUID NOT NULL REFERENCES habits ON DELETE CASCADE
reminder_time       TIME NOT NULL
days_of_week        SMALLINT DEFAULT 127  -- битовая маска дней недели
notification_text   TEXT
delivery_method     VARCHAR(20) DEFAULT 'push' CHECK(delivery_method IN ('push', 'email', 'sms'))
is_active           BOOLEAN DEFAULT true
created_at          TIMESTAMPTZ DEFAULT NOW()
```

**Уникальные типы:** UUID, TIME, SMALLINT, TEXT, VARCHAR, BOOLEAN, TIMESTAMPTZ = **7 типов** ✓---

### 9. habit_stats (агрегаты от триггера) — 9 колонок, 6 РАЗНЫХ типов ✓

```sql
id                  UUID PRIMARY KEY DEFAULT gen_random_uuid()
habit_id            UUID UNIQUE NOT NULL REFERENCES habits ON DELETE CASCADE
total_checkins      INTEGER DEFAULT 0
current_streak      INTEGER DEFAULT 0
longest_streak      INTEGER DEFAULT 0
completion_rate     NUMERIC(5,2) DEFAULT 0.00
average_mood        NUMERIC(3,2)  -- средний mood из checkins
last_checkin_at     DATE
updated_at          TIMESTAMPTZ DEFAULT NOW()
```

**Уникальные типы:** UUID, INTEGER, NUMERIC, DATE, TIMESTAMPTZ = **5 типов** ✓---

### 10. audit_log (аудит изменений) — 9 колонок, 6 РАЗНЫХ типов ✓

```sql
id              UUID PRIMARY KEY DEFAULT gen_random_uuid()
table_name      VARCHAR(100) NOT NULL
operation       VARCHAR(10) CHECK(operation IN ('INSERT', 'UPDATE', 'DELETE')) NOT NULL
record_id       UUID
user_id         UUID  -- из SET LOCAL app.user_id
old_data        JSONB
new_data        JSONB
ip_address      INET  -- IP адрес пользователя (если доступен)
changed_at      TIMESTAMPTZ DEFAULT NOW()

CREATE INDEX idx_audit_changed_at ON audit_log(changed_at DESC);
```

**Уникальные типы:** UUID, VARCHAR, JSONB, INET, TIMESTAMPTZ = **5 типов** ✓---

### 11. _manual_migrations (контроль SQL-миграций) — 6 колонок, 5 РАЗНЫХ типов ✓

```sql
id                  UUID PRIMARY KEY DEFAULT gen_random_uuid()
name                VARCHAR(255) UNIQUE NOT NULL
checksum            VARCHAR(64)  -- SHA-256 hash
applied_at          TIMESTAMPTZ DEFAULT NOW()
execution_time_ms   INTEGER
status              VARCHAR(20) DEFAULT 'success' CHECK(status IN ('success', 'failed'))
```

**Уникальные типы:** UUID, VARCHAR, TIMESTAMPTZ, INTEGER = **4 типа** ❌**ИСПРАВЛЕНИЕ:**

```sql
id                  UUID PRIMARY KEY DEFAULT gen_random_uuid()
name                VARCHAR(255) UNIQUE NOT NULL
checksum            CHAR(64)  -- SHA-256 hash фиксированной длины
applied_at          TIMESTAMPTZ DEFAULT NOW()
execution_time_ms   INTEGER
status              VARCHAR(20) DEFAULT 'success' CHECK(status IN ('success', 'failed'))
applied_by          TEXT  -- имя пользователя или 'system'
```

**Уникальные типы:** UUID, VARCHAR, CHAR, TIMESTAMPTZ, INTEGER, TEXT = **6 типов** ✓---

### 12. batch_import_jobs — 9 колонок, 6 РАЗНЫХ типов ✓

```sql
id              UUID PRIMARY KEY DEFAULT gen_random_uuid()
user_id         UUID REFERENCES users ON DELETE SET NULL
entity_type     VARCHAR(50) NOT NULL  -- 'habits', 'checkins', etc
status          VARCHAR(20) CHECK(status IN ('pending', 'processing', 'completed', 'failed'))
total_records   INTEGER DEFAULT 0
success_count   INTEGER DEFAULT 0
error_count     INTEGER DEFAULT 0
started_at      TIMESTAMPTZ DEFAULT NOW()
completed_at    TIMESTAMPTZ
```

**Уникальные типы:** UUID, VARCHAR, INTEGER, TIMESTAMPTZ = **4 типа** ❌**ИСПРАВЛЕНИЕ:**

```sql
id                  UUID PRIMARY KEY DEFAULT gen_random_uuid()
user_id             UUID REFERENCES users ON DELETE SET NULL
entity_type         VARCHAR(50) NOT NULL
status              VARCHAR(20) CHECK(status IN ('pending', 'processing', 'completed', 'failed'))
total_records       INTEGER DEFAULT 0
success_count       INTEGER DEFAULT 0
error_count         INTEGER DEFAULT 0
progress_percent    NUMERIC(5,2) DEFAULT 0.00
file_size_bytes     BIGINT
started_at          TIMESTAMPTZ DEFAULT NOW()
completed_at        TIMESTAMPTZ
```

**Уникальные типы:** UUID, VARCHAR, INTEGER, NUMERIC, BIGINT, TIMESTAMPTZ = **6 типов** ✓---

### 13. batch_import_errors — 7 колонок, 6 РАЗНЫХ типов ✓

```sql
id              UUID PRIMARY KEY DEFAULT gen_random_uuid()
job_id          UUID NOT NULL REFERENCES batch_import_jobs ON DELETE CASCADE
row_number      INTEGER
record_data     JSONB NOT NULL
error_message   TEXT NOT NULL
error_code      VARCHAR(50)
created_at      TIMESTAMPTZ DEFAULT NOW()
```

**Уникальные типы:** UUID, INTEGER, JSONB, TEXT, VARCHAR, TIMESTAMPTZ = **6 типов** ✓---**Итого типов в PostgreSQL:**

- UUID
- TEXT
- VARCHAR(N)
- CHAR(N)
- INTEGER
- BIGINT
- SMALLINT
- NUMERIC(p,s)
- BOOLEAN
- DATE
- TIME
- TIMESTAMPTZ
- JSONB
- INET

**Каждая таблица имеет >= 5 РАЗНЫХ типов ✓**---**SQL объекты — ИСПРАВЛЕНО**

### sql/001_triggers.sql

```sql
-- Таблица контроля ручных миграций
CREATE TABLE IF NOT EXISTS _manual_migrations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name VARCHAR(255) UNIQUE NOT NULL,
  checksum CHAR(64),
  applied_at TIMESTAMPTZ DEFAULT NOW(),
  execution_time_ms INTEGER,
  status VARCHAR(20) DEFAULT 'success' CHECK(status IN ('success', 'failed')),
  applied_by TEXT
);

-- ФУНКЦИЯ АУДИТА (ИСПРАВЛЕНО: OLD+NEW на UPDATE, правильный RETURN, user_id)
CREATE OR REPLACE FUNCTION audit_trigger_fn() RETURNS TRIGGER AS $$
DECLARE
  v_user_id UUID;
BEGIN
  -- Получаем user_id из SET LOCAL app.user_id (если есть)
  BEGIN
    v_user_id := nullif(current_setting('app.user_id', true), '')::UUID;
  EXCEPTION WHEN OTHERS THEN
    v_user_id := NULL;
  END;

  -- Записываем аудит
  INSERT INTO audit_log (table_name, operation, record_id, user_id, old_data, new_data, changed_at)
  VALUES (
    TG_TABLE_NAME::VARCHAR,
    TG_OP::VARCHAR,
    COALESCE(NEW.id, OLD.id),
    v_user_id,
    CASE WHEN TG_OP IN ('UPDATE', 'DELETE') THEN row_to_json(OLD)::jsonb ELSE NULL END,
    CASE WHEN TG_OP IN ('INSERT', 'UPDATE') THEN row_to_json(NEW)::jsonb ELSE NULL END,
    NOW()
  );

  -- Правильный RETURN
  IF TG_OP = 'DELETE' THEN
    RETURN OLD;
  ELSE
    RETURN NEW;
  END IF;
END;
$$ LANGUAGE plpgsql;

-- Триггеры аудита
CREATE TRIGGER habits_audit 
  AFTER INSERT OR UPDATE OR DELETE ON habits
  FOR EACH ROW EXECUTE FUNCTION audit_trigger_fn();

CREATE TRIGGER habit_checkins_audit
  AFTER INSERT OR UPDATE OR DELETE ON habit_checkins
  FOR EACH ROW EXECUTE FUNCTION audit_trigger_fn();

CREATE TRIGGER tags_audit
  AFTER INSERT OR UPDATE OR DELETE ON tags
  FOR EACH ROW EXECUTE FUNCTION audit_trigger_fn();

CREATE TRIGGER reminders_audit
  AFTER INSERT OR UPDATE OR DELETE ON reminders
  FOR EACH ROW EXECUTE FUNCTION audit_trigger_fn();

-- ТРИГГЕР АГРЕГАТОВ
CREATE OR REPLACE FUNCTION update_habit_stats() RETURNS TRIGGER AS $$
DECLARE
  v_total INTEGER;
  v_avg_mood NUMERIC;
BEGIN
  IF TG_OP = 'INSERT' THEN
    -- Вычисляем агрегаты
    SELECT COUNT(*), AVG(mood_rating)
    INTO v_total, v_avg_mood
    FROM habit_checkins
    WHERE habit_id = NEW.habit_id;
    
    -- Обновляем habit_stats
    INSERT INTO habit_stats (habit_id, total_checkins, average_mood, last_checkin_at, updated_at)
    VALUES (NEW.habit_id, v_total, v_avg_mood, NEW.checkin_date, NOW())
    ON CONFLICT (habit_id) DO UPDATE SET
      total_checkins = v_total,
      average_mood = v_avg_mood,
      last_checkin_at = GREATEST(habit_stats.last_checkin_at, NEW.checkin_date),
      updated_at = NOW();
      
  ELSIF TG_OP = 'DELETE' THEN
    -- Пересчитываем при удалении
    SELECT COUNT(*), AVG(mood_rating)
    INTO v_total, v_avg_mood
    FROM habit_checkins
    WHERE habit_id = OLD.habit_id;
    
    UPDATE habit_stats SET
      total_checkins = v_total,
      average_mood = v_avg_mood,
      updated_at = NOW()
    WHERE habit_id = OLD.habit_id;
  END IF;
  
  RETURN COALESCE(NEW, OLD);
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER habit_checkins_update_stats
  AFTER INSERT OR DELETE ON habit_checkins
  FOR EACH ROW EXECUTE FUNCTION update_habit_stats();
```



### sql/002_functions.sql

```sql
-- СКАЛЯРНАЯ: расчёт completion rate
CREATE OR REPLACE FUNCTION calc_completion_rate(
  p_user_id UUID,
  p_from DATE,
  p_to DATE
) RETURNS NUMERIC AS $$
DECLARE
  v_expected INTEGER;
  v_actual INTEGER;
BEGIN
  -- Ожидаемое кол-во (дни * активные привычки)
  SELECT COUNT(DISTINCT h.id) * (p_to - p_from + 1)
  INTO v_expected
  FROM habits h
  WHERE h.user_id = p_user_id AND NOT h.is_archived;
  
  -- Фактическое кол-во checkins
  SELECT COUNT(*)
  INTO v_actual
  FROM habit_checkins hc
  JOIN habits h ON h.id = hc.habit_id
  WHERE h.user_id = p_user_id
    AND hc.checkin_date BETWEEN p_from AND p_to;
  
  IF v_expected = 0 THEN
    RETURN 0;
  END IF;
  
  RETURN ROUND((v_actual::NUMERIC / v_expected) * 100, 2);
END;
$$ LANGUAGE plpgsql;

-- ТАБЛИЧНАЯ: отчёт по привычкам
CREATE OR REPLACE FUNCTION report_user_habits(
  p_user_id UUID,
  p_from DATE,
  p_to DATE
) RETURNS TABLE(
  habit_id UUID,
  habit_name TEXT,
  habit_type VARCHAR,
  total_checkins BIGINT,
  completion_rate NUMERIC,
  last_checkin DATE
) AS $$
BEGIN
  RETURN QUERY
  SELECT 
    h.id,
    h.name,
    h.type,
    COUNT(hc.id) as total_checkins,
    ROUND(
      (COUNT(hc.id)::NUMERIC / NULLIF((p_to - p_from + 1), 0)) * 100, 
      2
    ) as completion_rate,
    MAX(hc.checkin_date) as last_checkin
  FROM habits h
  LEFT JOIN habit_checkins hc ON hc.habit_id = h.id 
    AND hc.checkin_date BETWEEN p_from AND p_to
  WHERE h.user_id = p_user_id
  GROUP BY h.id, h.name, h.type
  ORDER BY total_checkins DESC;
END;
$$ LANGUAGE plpgsql;
```



### sql/003_views.sql

```sql
-- VIEW 1: Сводка по привычкам пользователя
CREATE OR REPLACE VIEW v_user_habit_summary AS
SELECT 
  u.id as user_id,
  u.email,
  u.full_name,
  COUNT(DISTINCT h.id) as total_habits,
  COUNT(DISTINCT CASE WHEN h.type = 'good' THEN h.id END) as good_habits,
  COUNT(DISTINCT CASE WHEN h.type = 'bad' THEN h.id END) as bad_habits,
  COALESCE(SUM(hs.total_checkins), 0) as total_checkins,
  MAX(hs.last_checkin_at) as last_activity
FROM users u
LEFT JOIN habits h ON h.user_id = u.id AND NOT h.is_archived
LEFT JOIN habit_stats hs ON hs.habit_id = h.id
GROUP BY u.id, u.email, u.full_name;

-- VIEW 2: Ежедневная статистика
CREATE OR REPLACE VIEW v_daily_completion AS
SELECT 
  hc.checkin_date,
  COUNT(*) as total_checkins,
  COUNT(DISTINCT hc.user_id) as active_users,
  COUNT(DISTINCT hc.habit_id) as habits_completed,
  ROUND(AVG(hc.mood_rating), 2) as avg_mood,
  SUM(hc.duration_minutes) as total_minutes
FROM habit_checkins hc
GROUP BY hc.checkin_date
ORDER BY hc.checkin_date DESC;

-- VIEW 3: Использование тегов
CREATE OR REPLACE VIEW v_tag_usage AS
SELECT 
  t.id as tag_id,
  t.name as tag_name,
  t.color,
  COUNT(ht.habit_id) as usage_count,
  COUNT(DISTINCT ht.habit_id) as unique_habits,
  MAX(ht.assigned_at) as last_used
FROM tags t
LEFT JOIN habit_tags ht ON ht.tag_id = t.id
GROUP BY t.id, t.name, t.color
ORDER BY usage_count DESC;
```



### sql/004_indexes.sql

```sql
CREATE INDEX IF NOT EXISTS idx_habits_user_id ON habits(user_id) WHERE NOT is_archived;
CREATE INDEX IF NOT EXISTS idx_habit_checkins_habit_date ON habit_checkins(habit_id, checkin_date DESC);
CREATE INDEX IF NOT EXISTS idx_habit_checkins_user_date ON habit_checkins(user_id, checkin_date DESC);
CREATE INDEX IF NOT EXISTS idx_habit_checkins_date ON habit_checkins(checkin_date DESC);
CREATE INDEX IF NOT EXISTS idx_audit_log_table_op_time ON audit_log(table_name, operation, changed_at DESC);
CREATE INDEX IF NOT EXISTS idx_audit_log_user_time ON audit_log(user_id, changed_at DESC) WHERE user_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_habit_tags_habit ON habit_tags(habit_id);
CREATE INDEX IF NOT EXISTS idx_habit_tags_tag ON habit_tags(tag_id);
```

**Seed — 800 habits, 10000 checkins**[`scripts/seed.ts`](packages/db/scripts/seed.ts):

```typescript
import { PrismaClient } from '@prisma/client';
import { faker } from '@faker-js/faker';

faker.seed(12345); // ФИКСИРОВАННЫЙ SEED

const prisma = new PrismaClient();

async function main() {
  console.log('🌱 Seeding database...');
  
  const users = await createUsers(100);
  const tags = await createTags(50);
  const habits = await createHabits(users, 800); // КРУПНАЯ ТАБЛИЦА
  await createHabitTags(habits, tags, 1500);
  await createSchedules(habits);
  await createReminders(habits, 600);
  await createCheckins(habits, users, 10000); // ТРАНЗАКЦИОННАЯ
  
  console.log('✅ Seeding completed!');
}
```

**Скрипт применения SQL — с контролем**[`scripts/apply-sql.ts`](packages/db/scripts/apply-sql.ts):

```typescript
import { PrismaClient } from '@prisma/client';
import fs from 'fs/promises';
import path from 'path';
import crypto from 'crypto';

const prisma = new PrismaClient();
const SQL_DIR = path.join(__dirname, '../sql');

const migrations = [
  '001_triggers.sql',
  '002_functions.sql',
  '003_views.sql',
  '004_indexes.sql'
];

async function applyMigration(filename: string) {
  const filepath = path.join(SQL_DIR, filename);
  const content = await fs.readFile(filepath, 'utf-8');
  const checksum = crypto.createHash('sha256').update(content).digest('hex');
  
  return prisma.$transaction(async (tx) => {
    const existing = await tx.$queryRaw<Array<{name: string}>>`
      SELECT name FROM _manual_migrations WHERE name = ${filename}
    `;
    
    if (existing.length > 0) {
      console.log(`⏭️  ${filename} already applied`);
      return;
    }
    
    console.log(`📝 Applying ${filename}...`);
    const startTime = Date.now();
    
    await tx.$executeRawUnsafe(content);
    
    const executionTime = Date.now() - startTime;
    
    await tx.$executeRaw`
      INSERT INTO _manual_migrations (name, checksum, execution_time_ms, applied_by)
      VALUES (${filename}, ${checksum}, ${executionTime}, 'system')
    `;
    
    console.log(`✅ ${filename} applied in ${executionTime}ms`);
  });
}
```



## Фаза 3: Backend на NestJS — ИСПРАВЛЕНО

**Prisma Middleware — КРИТИЧНО: БЕЗ SQL-КОНКАТЕНАЦИИ**[`prisma/prisma.service.ts`](apps/backend/src/prisma/prisma.service.ts):

```typescript
import { Injectable, OnModuleInit } from '@nestjs/common';
import { PrismaClient } from '@repo/db';
import { AsyncLocalStorage } from 'async_hooks';

export const asyncLocalStorage = new AsyncLocalStorage<{ userId?: string }>();

@Injectable()
export class PrismaService extends PrismaClient implements OnModuleInit {
  async onModuleInit() {
    await this.$connect();
    
    // Middleware: SET LOCAL app.user_id ПАРАМЕТРИЗОВАННО
    this.$use(async (params, next) => {
      const store = asyncLocalStorage.getStore();
      const userId = store?.userId;
      
      if (userId) {
        // ✅ ПРАВИЛЬНО: Параметризация через $executeRaw
        await this.$executeRaw`SET LOCAL app.user_id = ${userId}`;
        
        // ❌ НЕПРАВИЛЬНО (было):
        // await this.$executeRawUnsafe(`SET LOCAL app.user_id = '${userId}'`);
      }
      
      const result = await next(params);
      return result;
    });
  }
}
```

**Альтернативный вариант (если $executeRaw не работает с SET LOCAL):**

```typescript
// Если Prisma не поддерживает параметризацию для SET LOCAL,
// используем pg напрямую с параметризацией:
import { Client } from 'pg';

this.$use(async (params, next) => {
  const store = asyncLocalStorage.getStore();
  const userId = store?.userId;
  
  if (userId) {
    // Используем нативный драйвер с параметризацией
    const client = new Client({ connectionString: process.env.DATABASE_URL });
    await client.connect();
    
    // Параметризованный запрос
    await client.query('SET LOCAL app.user_id = $1', [userId]);
    await client.end();
  }
  
  return next(params);
});
```

**Ещё один вариант — использовать pg_catalog.set_config:**

```typescript
if (userId) {
  // ✅ ПАРАМЕТРИЗОВАНО через функцию PostgreSQL
  await this.$executeRaw`SELECT set_config('app.user_id', ${userId}, true)`;
}
```

**Auth Guard**[`auth/jwt-auth.guard.ts`](apps/backend/src/auth/jwt-auth.guard.ts):

```typescript
@Injectable()
export class JwtAuthGuard extends AuthGuard('jwt') {
  canActivate(context: ExecutionContext) {
    const request = context.switchToHttp().getRequest();
    const user = request.user;
    
    return asyncLocalStorage.run({ userId: user?.id }, () => {
      return super.canActivate(context);
    });
  }
}
```

**Остальные модули**

- Auth: POST `/auth/register`, POST `/auth/login`, GET `/auth/me`
- CRUD: habits, checkins, tags, reminders через Prisma
- Reports: GET `/reports/user/:userId` — параметризованный SQL
- Batch: POST `/batch-import` с валидацией и логированием
- Swagger: `/api/docs`

## Фаза 4: Адаптация фронтенда

**Удаление Supabase**

- Удалить `@supabase/*` из package.json
- Удалить `lib/supabase/`
- Удалить `app/api/auth/callback/`

**API клиент**[`lib/api/client.ts`](apps/frontend/lib/api/client.ts):

```typescript
const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';

export async function apiCall<T>(endpoint: string, options?: RequestInit): Promise<T> {
  const token = getToken(); // из cookie или localStorage
  
  const res = await fetch(`${API_URL}${endpoint}`, {
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...(token && { Authorization: `Bearer ${token}` }),
      ...options?.headers,
    },
  });
  
  if (!res.ok) throw new Error(`API error: ${res.statusText}`);
  return res.json();
}
```

**Server Actions**Переписать [`authActions.ts`](apps/frontend/app/actions/authActions.ts), [`habitActions.ts`](apps/frontend/app/actions/habitActions.ts), [`completionActions.ts`](apps/frontend/app/actions/completionActions.ts) для вызова backend API.**Middleware — БЕЗ сетевых вызовов**[`middleware.ts`](apps/frontend/middleware.ts):

```typescript
import { NextResponse } from 'next/server';
import type { NextRequest } from 'next/server';
import { jwtVerify } from 'jose';

export async function middleware(request: NextRequest) {
  const token = request.cookies.get('access_token')?.value;
  
  const isAuthPage = request.nextUrl.pathname.startsWith('/login') || 
                     request.nextUrl.pathname.startsWith('/signup');
  
  if (!token && !isAuthPage) {
    return NextResponse.redirect(new URL('/login', request.url));
  }
  
  // Быстрая проверка JWT (БЕЗ fetch к backend)
  if (token) {
    try {
      const secret = new TextEncoder().encode(process.env.JWT_SECRET);
      await jwtVerify(token, secret);
    } catch {
      if (!isAuthPage) {
        return NextResponse.redirect(new URL('/login', request.url));
      }
    }
  }
  
  if (token && isAuthPage) {
    return NextResponse.redirect(new URL('/', request.url));
  }
  
  return NextResponse.next();
}
```



## Фаза 5: Docker Compose — ИСПРАВЛЕНО

**docker-compose.yml**

```yaml
version: '3.9'

services:
  db:
    image: postgres:16-alpine
    environment:
      POSTGRES_DB: habit_tracker
      POSTGRES_USER: postgres
      POSTGRES_PASSWORD: postgres
    ports:
    - "5432:5432"
    volumes:
    - db_data:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U postgres"]
      interval: 5s
      timeout: 5s
      retries: 5

  backend:
    build:
      context: .
      dockerfile: apps/backend/Dockerfile
    environment:
      DATABASE_URL: postgresql://postgres:postgres@db:5432/habit_tracker
      JWT_SECRET: ${JWT_SECRET:-change-me}
      PORT: 3001
    ports:
    - "3001:3001"
    depends_on:
      db:
        condition: service_healthy

  frontend:
    build:
      context: .
      dockerfile: apps/frontend/Dockerfile
    environment:
      # ✅ В Docker используем имя сервиса
      NEXT_PUBLIC_API_URL: http://backend:3001
      API_URL: http://backend:3001
      JWT_SECRET: ${JWT_SECRET:-change-me}
    ports:
    - "3000:3000"
    depends_on:
    - backend

volumes:
  db_data:
```

**Environment файлы**[`.env.example`](.env.example):

```javascript
JWT_SECRET=your-secret-key-here
DATABASE_URL=postgresql://postgres:postgres@localhost:5432/habit_tracker
```

[`apps/frontend/.env.local.example`](apps/frontend/.env.local.example):

```javascript
# Для локальной разработки ВНЕ Docker
NEXT_PUBLIC_API_URL=http://localhost:3001
API_URL=http://localhost:3001
JWT_SECRET=your-secret-key
```

[`apps/frontend/.env.docker`](apps/frontend/.env.docker):

```javascript
# Для Docker Compose (используется в Dockerfile)
NEXT_PUBLIC_API_URL=http://backend:3001
API_URL=http://backend:3001
JWT_SECRET=your-secret-key
```



## Фаза 6: Анализ производительности

[`docs/perf.md`](docs/perf.md) — EXPLAIN ANALYZE до/после индексов (минимум 2 запроса).

## Фаза 7: Документация

- [`docs/requirements-checklist.md`](docs/requirements-checklist.md)
- [`docs/schema.md`](docs/schema.md) — **таблицы с явным перечислением типов**
- [`docs/api.md`](docs/api.md)
- [`docs/perf.md`](docs/perf.md)
- [`docs/batch-import.md`](docs/batch-import.md)
- [`docs/security-notes.md`](docs/security-notes.md)
- [`README.md`](README.md)

---

## Definition of Done (ФИНАЛЬНЫЙ)

### Структура и типы данных

✅ 13 таблиц в БД✅ **КАЖДАЯ таблица имеет >= 5 РАЗНЫХ типов** (UUID, TEXT, VARCHAR, INTEGER, BOOLEAN и т.д.)✅ Проверка: `docs/schema.md` содержит таблицу с перечислением колонок и уникальных типов для каждой таблицы✅ Ограничения: PK, FK (CASCADE), UNIQUE, CHECK, NOT NULL

### Данные

✅ Seed: 800 habits, 10000 checkins, faker.seed(12345)✅ Проверка: `SELECT COUNT(*) FROM habits` = ~800, `SELECT COUNT(*) FROM habit_checkins` = ~10000

### SQL объекты

✅ Аудит-триггер с user_id (UPDATE записывает old+new, правильный RETURN)✅ Проверка: добавить habit через API → `SELECT * FROM audit_log WHERE table_name='habits' ORDER BY changed_at DESC LIMIT 1` показывает user_id✅ Триггер агрегатов: habit_stats обновляется✅ Проверка: добавить checkin → `habit_stats.total_checkins` увеличивается✅ Функции: calc_completion_rate (скалярная), report_user_habits (табличная)✅ Проверка: `SELECT calc_completion_rate('<uuid>', '2024-01-01', '2024-12-31')`✅ VIEW: v_user_habit_summary, v_daily_completion, v_tag_usage✅ Проверка: `SELECT * FROM v_user_habit_summary LIMIT 5`✅ Индексы созданы

### Контроль миграций

✅ Таблица `_manual_migrations` существует✅ Повторный запуск `pnpm db:sql` не применяет миграции заново

### Backend — КРИТИЧНО

✅ **Prisma middleware БЕЗ SQL-конкатенации:**

- Используется `$executeRaw` с параметризацией ИЛИ `set_config()`  
- НЕ используется `$executeRawUnsafe` с интерполяцией строк  

✅ Проверка: в `prisma.service.ts` НЕТ конструкций вида `` `SET ... = '${variable}'` ``✅ Отчётные эндпойнты: `$queryRaw` с параметрами✅ Проверка: в коде НЕТ SQL-конкатенации (grep в backend/src/)✅ Batch import с логированием✅ Swagger: http://localhost:3001/api/docs

### Frontend

✅ НЕТ Supabase зависимостей✅ Middleware БЕЗ сетевых вызовов (только jwtVerify)✅ Проверка: в `middleware.ts` НЕТ `fetch()` или `apiCall()`

### Docker

✅ docker-compose работает✅ Frontend в docker использует `NEXT_PUBLIC_API_URL=http://backend:3001`✅ Локальный dev использует `NEXT_PUBLIC_API_URL=http://localhost:3001`✅ Секреты в .env, есть .env.example

### Производительность

✅ `docs/perf.md` с EXPLAIN ANALYZE (до/после индексов, минимум 2 запроса)

### Документация

✅ `docs/schema.md` содержит:

- Список таблиц
- Для каждой таблицы: колонки + типы + **подсчёт уникальных типов >= 5**
- ER-диаграмма (Mermaid)  

✅ Остальные docs/ файлы

### Финальная проверка

```bash
# 1. Проверка типов в схеме
cat docs/schema.md | grep "Уникальные типы"
# Каждая таблица должна показать >= 5 разных типов

# 2. Проверка параметризации в Prisma middleware
grep -n "executeRawUnsafe.*app.user_id" apps/backend/src/prisma/prisma.service.ts
# Результат: ничего не найдено (если найдено — ОШИБКА!)

grep -n "executeRaw.*app.user_id" apps/backend/src/prisma/prisma.service.ts
# Результат: строка с параметризованным запросом

# 3. Проверка SQL-конкатенации в backend
grep -rn "\\${\|\\+" apps/backend/src/ | grep -i "select\|insert\|update\|delete" | grep -v "test"
# Результат: не должно быть SQL с конкатенацией

# 4. Запуск
pnpm install
pnpm docker:up
pnpm db:migrate
pnpm db:sql
pnpm db:seed

# 5. Проверка данных
docker exec -it habit_tracker_db psql -U postgres -d habit_tracker -c "
  SELECT table_name, COUNT(*) FROM (
    SELECT 'users' as table_name, COUNT(*) FROM users
    UNION ALL SELECT 'habits', COUNT(*) FROM habits
    UNION ALL SELECT 'habit_checkins', COUNT(*) FROM habit_checkins
  ) t GROUP BY table_name;
"

# 6. Проверка аудита с user_id
# Создать habit через Swagger → проверить audit_log
docker exec -it habit_tracker_db psql -U postgres -d habit_tracker -c "
  SELECT user_id, operation, changed_at 
  FROM audit_log 
  WHERE table_name = 'habits' 
  ORDER BY changed_at DESC 
  LIMIT 5;
"
# user_id НЕ должен быть NULL

# 7. Проверка VIEW
docker exec -it habit_tracker_db psql -U postgres -d habit_tracker -c "
  SELECT * FROM v_user_habit_summary LIMIT 3;
"

# 8. Проверка функций
docker exec -it habit_tracker_db psql -U postgres -d habit_tracker -c "
  SELECT * FROM report_user_habits(
    (SELECT id FROM users LIMIT 1),
    '2024-01-01'::date,
    '2024-12-31'::date
  );
"

```