...
export async function setCompletionsBatch(
	...
	let results: Array<{ date: string; completed: boolean; ok: boolean }> = [];

	try {
		results = await serverCheckinsApi.batchUpdate(habitId, updates);
	} catch (error) {
		console.error("Error setting completion batch:", error);
