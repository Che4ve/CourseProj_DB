...
export const serverTagsApi = {
	async getAll(): Promise<TagWithOwnership[]> {
		return fetchApiServer<TagWithOwnership[]>("/tags", {
			next: { tags: ["tags-list"] },
		});
	},
	...
	async update(id: string, data: UpdateTagDto): Promise<TagWithOwnership> {
		return fetchApiServer<TagWithOwnership>(`/tags/${id}`, {
			method: "PATCH",
			body: JSON.stringify(data),
		});
	},
...
export const serverReportsApi = {
	...
	async getCompletionRate(from?: string, to?: string) {
		...
		if (to) params.set("to", to);
		const query = params.toString();
		return fetchApiServer<{ completionRate: number }>(
			`/reports/completion-rate${query ? `?${query}` : ""}`,
		);
	},
