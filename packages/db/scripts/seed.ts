			...
			data: {
				...
				profile: {
					create: {
						...
						avatarUrl: faker.datatype.boolean(0.6)
							? faker.image.avatar()
							: null,
						timezone: faker.location.timeZone(),
						dateOfBirth: faker.datatype.boolean(0.8)
...
async function createTags(count: number) {
	console.log(`Creating ${count} tags...`);
	const tags = [];
	...
	const habitTemplates = [
		{
			...
			type: "good",
			desc: "Start the day with physical activity",
		},
		...
		if (faker.datatype.boolean(0.7)) {
			await prisma.habitSchedule.create({
				data: {
					...
					endDate: faker.datatype.boolean(0.3)
		...
		await prisma.reminder.create({
			data: {
				...
				reminderTime: new Date(
					`2024-01-01T${faker.number.int({ min: 6, max: 22 }).toString().padStart(2, "0")}:${faker.number.int({ min: 0, max: 59 }).toString().padStart(2, "0")}:00`,
				),
				daysOfWeek: faker.number.int({ min: 1, max: 127 }),
				notificationText: faker.datatype.boolean(0.7)
			...
			const checkin = await prisma.habitCheckin.create({
				data: {
					...
					checkinTime: new Date(
						`2024-01-01T${faker.number.int({ min: 6, max: 23 }).toString().padStart(2, "0")}:${faker.number.int({ min: 0, max: 59 }).toString().padStart(2, "0")}:00`,
					),
					notes: faker.datatype.boolean(0.3) ? faker.lorem.sentence() : null,
					moodRating: faker.datatype.boolean(0.8)
						? faker.number.int({ min: 1, max: 5 })
						: null,
					durationMinutes: faker.datatype.boolean(0.6)
