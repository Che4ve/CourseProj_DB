...
Сервисы трекинга привычек требуют надежного хранения данных, аналитики и контроля изменений. Поэтому важно спроектировать базу данных, которая поддерживает целостность, аудит и быструю выборку. Требования к структуре пояснительной записки определены ГОСТ 7.32-2017 \cite{gost732} и техническим заданием \cite{courseTask}.
...
Для достижения цели нужно было:
\begin{enumerate}
...
Практическая ценность работы --- готовая БД и API для сервиса трекинга привычек. Отчет включает введение, аналитическую, проектную и технологическую части, заключение и приложения.
...
\begin{longtable}{|>{\raggedright\arraybackslash}p{2.6cm}|>{\raggedright\arraybackslash}p{3.3cm}|>{\raggedright\arraybackslash}p{4.1cm}|>{\raggedright\arraybackslash}p{6.0cm}|}
\hline
...
\begin{longtable}{|>{\raggedright\arraybackslash}p{3.4cm}|>{\raggedright\arraybackslash}p{3.3cm}|>{\raggedright\arraybackslash}p{4.1cm}|>{\raggedright\arraybackslash}p{5.4cm}|}
\hline
\textbf{Колонка} & \textbf{Тип} & \textbf{Ограничения} & \textbf{Описание} \\
\hline
\endfirsthead
...
\begin{longtable}{|>{\raggedright\arraybackslash}p{3.3cm}|>{\raggedright\arraybackslash}p{3.3cm}|>{\raggedright\arraybackslash}p{4.1cm}|>{\raggedright\arraybackslash}p{5.4cm}|}
\hline
\textbf{Колонка} & \textbf{Тип} & \textbf{Ограничения} & \textbf{Описание} \\
\hline
\endfirsthead
...
\begin{longtable}{|>{\raggedright\arraybackslash}p{3.3cm}|>{\raggedright\arraybackslash}p{3.3cm}|>{\raggedright\arraybackslash}p{4.1cm}|>{\raggedright\arraybackslash}p{5.2cm}|}
\hline
\textbf{Колонка} & \textbf{Тип} & \textbf{Ограничения} & \textbf{Описание} \\
\hline
\endfirsthead
...
Базовые CRUD-операции реализованы через Prisma ORM, а сложные выборки и отчеты --- через параметризованные SQL-запросы. Реализованы триггеры аудита и автоматического обновления агрегированной статистики, скалярная функция \texttt{calc\_completion\_rate} и табличная функция \texttt{report\_user\_habits}. Основные SQL-скрипты:
\begin{itemize}
  \item \texttt{packages/db/sql/001\_triggers.sql} --- триггеры аудита и агрегатов;
  \item \texttt{packages/db/sql/002\_functions.sql} --- функции;
  \item \texttt{packages/db/sql/003\_views.sql} --- представления;
  \item \texttt{packages/db/sql/004\_indexes.sql} --- индексы.
\end{itemize}
...
Для повышения производительности созданы индексы на полях, используемых в фильтрации и сортировке. В таблице \ref{tab:perf} приведено сравнение времени выполнения запросов до и после индексирования. Полные планы EXPLAIN ANALYZE приведены в файле \texttt{docs/perf.md}. Основные результаты: 52x быстрее для истории отметок и 69x быстрее для журнала аудита, среднее ускорение около 60x.
...
Ключевые индексы: \texttt{idx\_habits\_user\_id}, \texttt{idx\_habit\_checkins\_user\_date}, \texttt{idx\_audit\_log\_user\_time} и др.
...
В проекте разработана информационная система для трекинга привычек. Реализована реляционная БД из 13 таблиц со связями 1:1, 1:N и N:M, ограничениями целостности, триггерами аудита и агрегатов, функциями и представлениями. Реализован REST API и batch import. Оптимизация запросов индексами дала ускорение в десятки раз.

Задачи выполнены. Система корректно работает и может развиваться дальше за счет расширения аналитики и функциональности.
...
\begin{tikzpicture}[
    ...
    rel/.style={midway, fill=white, inner sep=1pt, font=\scriptsize}
]
    \node[table] (profiles) {
        user\_profiles
        \nodepart{second}
    ...
    \draw[edge] (users) -- node[rel, above, yshift=2pt] {1:1} (profiles);
    \draw[edge] (users) -- node[rel, above, yshift=2pt] {1:N} (habits);
    \draw[edge] (users) -- node[rel, left, xshift=-6pt] {1:N} (checkins);
    \draw[edge] (users) -- node[rel, left, xshift=-6pt] {1:N} (batch_jobs);
    \draw[edge] (users) -- node[rel, left, xshift=-6pt] {1:N} (audit);
    \draw[edge] (habits) -- node[rel, right, xshift=6pt] {1:N} (checkins);
    \draw[edge] (habits) -- node[rel, right, xshift=6pt] {1:N} (schedules);
    \draw[edge] (habits) -- node[rel, right, xshift=6pt] {1:N} (reminders);
    \draw[edge] (habits) -- node[rel, right, xshift=6pt] {1:1} (stats);
    \draw[edge] (habits) -- node[rel, right, xshift=6pt] {1:N} (habit_tags);
    \draw[edge] (tags) -- node[rel, right, xshift=6pt] {1:N} (habit_tags);
    \draw[edge] (batch_jobs) -- node[rel, left, xshift=-6pt] {1:N} (batch_errors);
\end{tikzpicture}}
