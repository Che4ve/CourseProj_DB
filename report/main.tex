...
\begin{itemize}
    ...
    \item \textbf{Представления:} \texttt{v\_user\_habit\_summary} для сводной статистики по привычкам пользователя, \texttt{v\_daily\_completion} для визуализации ежедневной активности и \texttt{v\_tag\_usage} для анализа использования тегов.
\end{itemize}
...
Для проверки производительности БД заполнена тестовыми данными.
\begin{table}[H]
...
В ходе проекта разработана информационная система для трекинга привычек. Спроектированная БД обеспечивает надежное хранение и аудит данных. Использование триггеров и функций позволило вынести часть бизнес-логики на сторону СУБД, а индексация обеспечила высокую скорость работы.\\

С целью демонстрации было реализовано веб-приложение с backend на NestJS и frontend на Next.js, включающее REST API с документацией через Swagger. В результате получена система, демонстрирующая достаточно полное применение возможностей PostgreSQL: триггеры, функции, представления, индексы и ограничения целостности.
